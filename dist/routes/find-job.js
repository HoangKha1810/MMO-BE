...
async function tableExists(table) {
    const rows = await prisma.$queryRawUnsafe(`SHOW TABLES LIKE ?`, table).catch(() => []);
    return rows.length > 0;
}
async function resolveFindJobTable() {
...
async function ensureFindJobColumns(table) {
...
async function insertFiltered(table, data) {
    const columns = await getTableColumns(table);
    const fields = Object.keys(data).filter((field) => columns.has(field));
    if (!fields.length) {
        throw new Error('Không có field hợp lệ để tạo tin');
    }
    await prisma.$executeRawUnsafe(`INSERT INTO \`${table}\` (${fields.map((field) => `\`${field}\``).join(', ')}) VALUES (${fields.map(() => '?').join(', ')})`, ...fields.map((field) => data[field]));
}
async function updateFiltered(table, id, userField, userId, data) {
    const columns = await getTableColumns(table);
    const fields = Object.keys(data).filter((field) => columns.has(field));
    if (!fields.length) {
        throw new Error('Không có field hợp lệ để cập nhật tin');
    }
    await prisma.$executeRawUnsafe(`UPDATE \`${table}\` SET ${fields.map((field) => `\`${field}\` = ?`).join(', ')} WHERE id = ? AND \`${userField}\` = ?`, ...fields.map((field) => data[field]), id, userId);
}
async function createOrUpdateFindJob(userId, body) {
...
router.post('/jobs', async (req, res) => {
    ...
    try {
        ...
        if (action === 'delete') {
            ...
            await prisma.$executeRawUnsafe(table === 'find_job_jobs'
                ? 'UPDATE `find_job_jobs` SET status = ?, updated_at = NOW() WHERE id = ? AND posted_by = ?'
                : 'UPDATE `find_jobs` SET status = ?, updated_at = NOW() WHERE id = ? AND user_id = ?', 'closed', jobId, userId);
...
router.post('/apply', async (req, res) => {
    ...
    try {
        ...
        const ownerRows = await prisma.$queryRawUnsafe(table === 'find_job_jobs'
            ? 'SELECT posted_by AS owner_id FROM `find_job_jobs` WHERE id = ? LIMIT 1'
            : 'SELECT user_id AS owner_id FROM `find_jobs` WHERE id = ? LIMIT 1', jobId);
        if (!ownerRows.length) {
      ...
      `, jobId, userId);
        await prisma.$executeRawUnsafe('UPDATE `find_job_jobs` SET application_count = (SELECT COUNT(*) FROM find_job_applications WHERE job_id = ?) WHERE id = ?', jobId, jobId).catch(() => undefined);
...
router.post('/report', async (req, res) => {
    ...
    try {
        ...
        VALUES (?, ?, ?, ?, NOW(), 0)
      `, jobId, userId, reason.slice(0, 255), note.slice(0, 2000));
